...
def mmasub(m: int, n: int, iter: int, xval: np.ndarray, xmin: np.ndarray, xmax: np.ndarray,
    ...
    P = np.maximum(dfdx, 0)
    Q = np.maximum(-dfdx, 0)
    # print('xmami: ', xmami.shape)
    # print('P: ', P.shape)
    # print('np.dot(eeem, xmami_inv.T): ', np.dot(eeem, xmami_inv.T).shape)
    ...
    # print("np.dot(P, ux_inv):", np.dot(Q, xl_inv).shape)
...
def subsolv(m: int, n: int, epsimin: float, low: np.ndarray, upp: np.ndarray, alfa: np.ndarray, 
    ...
    while epsi > epsimin:
        ...
        # print("gvec shape:", gvec.shape)
        # print("a shape:", a.shape if hasattr(a, 'shape') else "scalar")  # Check if 'a' has shape (if it's not a scalar)
        ...
        while (residumax > 0.9 * epsi) and (ittt < 200): #200
            # print(residumax)
            ittt += 1
            itera += 1
            ux1 = upp - x
            ...
            if m < n:
                ...
                # AA_reg = eigvecs @ np.diag(eigvals_clamped) @ eigvecs.T
                # solut = np.linalg.solve(AA_reg, bb)
                dlam = solut[0:m]
                dz = solut[m:m + 1]
